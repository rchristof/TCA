...
public class CalculateStatisticHelper {
	...
	private void calcScores() {
	    ...
	    data.setMaxSpeedTarget(hitList.stream()
	        .filter(i -> i.getType().equals(C.T_STAR))
	        .mapToLong(HitEntity::getSpeed)
	        .reduce(0L, (a, v) -> Math.max(a, v)));

	    data.setMinSpeedTarget(hitList.stream()
	        .filter(i -> i.getType().equals(C.T_STAR) && i.getSpeed() > 0)
	        .mapToLong(HitEntity::getSpeed)
	        .reduce(99999L, (a, v) -> Math.min(a, v)));
	    ...
	    data.setRcPercent(data.getTotalTarget() != 0 
	        ? String.valueOf(BigDecimal.valueOf(data.getTotalTargetRight() * 100.0 / data.getTotalTarget()).intValue()) + "%" 
	        : "n/d");

	    data.setRoScore(data.getTotalTargetWrong().toString());
	    data.setRoPercent(data.getTotalTarget() != 0 
	        ? String.valueOf(BigDecimal.valueOf(data.getTotalTargetWrong() * 100.0 / data.getTotalTarget()).intValue()) + "%" 
	        : "n/d");

	    data.setRiScore(data.getTotalNonTargetWrong().toString());
	    data.setRiPercent(data.getTotalNonTarget() != 0 
	        ? String.valueOf(BigDecimal.valueOf(data.getTotalNonTargetWrong() * 100.0 / data.getTotalNonTarget()).intValue()) + "%" 
	        : "n/d");

	    double media = totalSpeedHitCount != 0 ? totalSpeedHit * 1.0 / totalSpeedHitCount : -1;
	    if (media >= 0) {
	        double var = hitList.stream()
	            .filter(i -> i.getSpeed() > 0)
	            .mapToDouble(i -> i.getSpeed() - media)
	            .map(v -> v * v)
	            .sum();
	    ...
	    double mediaRc = totalSpeedHitCountRc != 0 ? totalSpeedHitRc * 1.0 / totalSpeedHitCountRc : -1;
	    if (mediaRc >= 0) {
	        double var = hitList.stream()
	            .filter(i -> i.getType().equals(C.T_STAR) && i.getSpeed() > 0)
	            .mapToDouble(i -> i.getSpeed() - mediaRc)
	            .map(v -> v * v)
	            .sum();
	    ...
	    if (mediaRi >= 0) {
	        double var = hitList.stream()
	            .filter(i -> i.getType().equals(C.T_BALLON) && i.getSpeed() > 0)
	            .mapToDouble(i -> i.getSpeed() - mediaRi)
	            .map(v -> v * v)
	            .sum();
